...
const OrderModal = () => {
    ...
    return (
        <div className={style.order_info}>
            ...
            <p className={`${style.title} text text_type_main-medium mt-10`}>{order?.name}</p>
            <p className={`${style.status} text text_type_main-default mt-3`}>{order?.status === 'done' ? 'Выполнен' : 'Готовится'}</p>
            <h3 className={`${style.title} text text_type_main-medium mt-15`}>Состав:</h3>
                    ...
                    .map((item) =>
                        <li className={style.item} key={item._id}>
                            <div className={style.image_container}>
                                ...
                                <p className={`${style.text} text_type_main-default`}>{item.name}</p>
                            </div>
