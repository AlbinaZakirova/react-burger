...
const OrderTemplate: FC<TOrderTemplate> = ({order}) => {
    ...
    return (
        <Link to={{pathname: location.pathname === '/feed' ? `/feed/${order._id}` : `/profile/orders/${order._id}`}}
            ...
            <li className={style.order_container}>
                ...
                <div className={style.bottom}>
                    <ul className={style.order_images}>
                        {orderIngredientsForImage.slice(-8).map((image) =>
                            <li className={style.image__container} key={image._id}>
